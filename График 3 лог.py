...
- логарифмическая шкала влияет только на отображение оси Y, а не на расчёт.
"""
...
LOG_TICKS = [240, 360, 480, 720, 1080, 1440, 2160, 3840, 5000]
...
def apply_log_y_ticks(ax, y_bot, y_top):
    ...
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f'{int(v):,}' if y_bot <= v <= y_top else ''))
...
for res, name in VIDEO_STANDARDS:
    if Y_BOT < res < Y_TOP:
        ...
        ax.axhline(res, color='#A0A0A0', lw=1.1, ls='-.', alpha=line_alpha, zorder=2)
        ax.text(
            0.85,
            text_y,
            name,
...
ax.plot(mos_points, widths, 'o', color='white', markersize=16, markeredgecolor='white', markeredgewidth=0, zorder=5)
ax.plot(mos_points, widths, 'o', color=MASTER_COLOR, markersize=11, markeredgecolor='white', markeredgewidth=2.5, zorder=6)
...
ax.set_ylabel('Нормированное пороговое разрешение, ширина (пикс., лог. шкала)', fontsize=11, labelpad=12, fontweight='bold', color='#222222')

ax.legend(loc='upper right', fontsize=10, framealpha=1.0, edgecolor='#CCCCCC', facecolor='white')

print('\nДиагностика интерполяции Лагранжа для графика 3 LOG:')
...
OUT_PNG = 'qoe_step3_7levels_lagrange_uhd_log.png'
plt.savefig(OUT_PNG, dpi=180, bbox_inches='tight', facecolor='white', edgecolor='none')
plt.show()

print(f'\n✓ Логарифмический график 3 успешно сгенерирован и сохранён как: {OUT_PNG}')

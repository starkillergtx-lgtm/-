# -*- coding: utf-8 -*-
"""
График 4 — итоговая перцептивная QoE-лестница, 6 уровней.
...
- уровень MOS=2.5 рассчитан диагностически, но исключён из итоговой лестницы;
...
# Итоговая 6-уровневая лестница: MOS=2.5 исключён.
mos_points = np.array([5.0, 4.5, 4.0, 3.5, 3.0, 2.0])
height_labels = np.array([2109, 1100, 551, 322, 272, 154])

# Диагностический исключённый уровень.
excluded_mos = 2.5
excluded_height = 263
...
def lagrange_eval(x_points, y_points, x_eval):
    ...
    - промежуточные субуровни MOS=4.5 и 3.5 рассчитаны по этому полиному;
    - уровень MOS=2.5 рассчитан диагностически, но исключён из итоговой лестницы.
    """
...
# Рассчитываем итоговые уровни от основной Main-кривой.
...
excluded_width = lagrange_value_at(base_mos_points, base_widths, excluded_mos)
excluded_label = f'{int(round(excluded_width))}×{int(excluded_height)}'

# Сохраняем таблицу уровней для проверки.
levels_rows = []
for mos, w, h, label in zip(mos_points, widths, height_labels, labels):
    levels_rows.append({
        'MOS': mos,
        'width_px_norm_exact': w,
        'width_px_rounded': int(round(w)),
        'height_px_label': int(h),
        'label': label,
        'included_in_final_ladder': True,
        'note': 'итоговый уровень',
    })

levels_rows.append({
    'MOS': excluded_mos,
    'width_px_norm_exact': excluded_width,
    'width_px_rounded': int(round(excluded_width)),
    'height_px_label': int(excluded_height),
    'label': excluded_label,
    'included_in_final_ladder': False,
    'note': 'исключён из итоговой лестницы',
})

levels_df = pd.DataFrame(levels_rows).sort_values('MOS', ascending=False)
levels_df.to_csv('qoe_step4_final_6levels_uhd_levels.csv', index=False, encoding='utf-8-sig')
...
X_R, X_L = 5.55, 0.8
Y_BOT, Y_TOP = 150, 4000
ax.set_xlim(X_R, X_L)
ax.set_ylim(Y_BOT, Y_TOP)
ax.set_yscale('log')

LOG_TICKS = [240, 360, 480, 720, 1080, 1440, 2160, 3840]
VIDEO_STANDARDS_LOG = VIDEO_STANDARDS + [
    (240, '240×135 (нижняя граница лог. шкалы)'),
]

# ── 4. Справочные линии видеостандартов ────────────────────────────────
for res, name in VIDEO_STANDARDS_LOG:
    if Y_BOT < res < Y_TOP:
        if res == 3840:
            line_alpha = 0.55
            text_y = res * 0.965
...
ax.plot(
    ...
    color=MASTER_COLOR,
    lw=3.5,
    label='Итоговая перцептивная лестница разрешений',
    zorder=4,
)
...
# Белый ореол под маркерами для читаемости на логарифмической сетке.
ax.plot(
    mos_points,
    widths,
    'o',
...
ax.set_title(
    ...
    'Интерполяция: полином Лагранжа 3-й степени по 4 опорным точкам; MOS=2.5 исключён; логарифмическая шкала Y',
    fontsize=13,
    fontweight='bold',
    pad=22,
    color='#111111',
...
ax.set_ylabel(
    'Пороговая ширина деградированного 4K-видео, приведённая к UHD-шкале, пикс.',
    fontsize=11,
    labelpad=12,
    fontweight='bold',
    color='#222222',
...
print("\nДиагностика интерполяции Лагранжа для графика 4 LOG:")
print("  Опорные точки: MOS=5, 4, 3, 2")
print("  Субуровни MOS=4.5 и MOS=3.5 рассчитаны по полиному")
print("  PCHIP / сплайн: НЕ используется")
print("  MOS=2.5 рассчитан диагностически, но исключён из итоговой лестницы")
print("  Логарифмическая шкала влияет только на отображение оси Y, не на расчёт")
for mos_q in [4.5, 3.5]:
    w_q = lagrange_value_at(base_mos_points, base_widths, mos_q)
    print(f"  MOS={mos_q:.1f}: ширина ≈ {w_q:.1f} px, округление: {int(round(w_q))} px")
print(f"  MOS=2.5: ширина ≈ {excluded_width:.1f} px, округление: {int(round(excluded_width))} px — исключён")
print(f"  Верхняя граница шкалы: {DISPLAY_MAX_WIDTH}×{DISPLAY_MAX_HEIGHT}; линия 4096×2160 не используется")

plt.tight_layout()
plt.savefig(
    'qoe_step4_final_6levels_lagrange_uhd_log.png',
    dpi=180,
    bbox_inches='tight',
    facecolor='white',
    edgecolor='none',
...
print("\n✓ График 4 LOG сохранён: qoe_step4_final_6levels_lagrange_uhd_log.png")
print("✓ Таблица уровней сохранена: qoe_step4_final_6levels_uhd_levels.csv")
if files is not None:
    files.download('qoe_step4_final_6levels_lagrange_uhd_log.png')
    files.download('qoe_step4_final_6levels_uhd_levels.csv')

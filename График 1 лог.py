...
def draw_itu(ax, y_bot, y_top):
    ...
    for res, label in ITU_LEVELS:
        if y_bot < res < y_top:
            ...
            # поэтому делаем линии светлее и разводим подписи по вертикали.
            if res == 4096:
                line_alpha = 0.45
                text_y = res * 1.035
                text_va = 'bottom'
...
def draw_single(ax, fmt, show_ylabel=True):
    ...
    Y_BOT = 150
    Y_TOP = int(np.ceil(max(y_valid) * 1.35 / 500.0) * 500)
    ...
    # Белый ореол под маркером, чтобы точки не терялись на линиях 4096/3840
    ax.plot(
        xv,
        yv,
        marker='o',
    ...
    for xi, yi, lbl in zip(xv, yv, labels):
        ...
        # Верхние подписи MOS=5 опускаем вниз, чтобы они не упирались в верхнее поле.
        if xi == 5:
            offset = (0, -26)
            va_pos = 'top'
        else:
    ...
    if show_ylabel:
        ax.set_ylabel('Пороговое разрешение (ширина, пикс., лог. шкала)', fontsize=10, labelpad=8)

# ── 8. Функция построения сводного графика ────────────────────
def draw_combined(ax):
    ...
    for fmt in FORMATS:
        ...
        for xi, yi, mos in zip(xv, yv, MOS_SCORES):
            ...
            # Верхние подписи MOS=5 также опускаем вниз.
            if mos == 5:
                if fmt == '720':
                    dx, dy = -10, -24
                    ha, va = 'right', 'top'
...
fig.suptitle(
    'QoE-лестница: субъективная оценка качества видео vs пороговое разрешение (логарифмическая шкала)\n'
...
print("\n✓ Логарифмический вариант сохранён: qoe_ultimate_logscale.png")
files.download('qoe_ultimate_logscale.png')
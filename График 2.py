# -*- coding: utf-8 -*-
"""
График 2 — формирование базовой QoE-лестницы по логике графика 1.

Ключевая логика:
- исходные данные qoe_ultimate.csv содержат разрешения деградированного
  исходного видео в шкале 4096×2160;
- в экспериментальной интерпретации верхняя 4K-граница приводится к UHD 3840×2160;
- поэтому координата графика нормируется одинаково для всех тестовых режимов:
      width_norm = width_raw × 3840 / 4096
      height_norm = height_raw
- режимы 4K / FHD / HD НЕ используются как разные базы пересчёта координаты;
  это условия вывода/просмотра в сессии, а не отдельная шкала Y;
...
DISPLAY_MAX_WIDTH = 3840     # корректная верхняя граница UHD 4K
DISPLAY_MAX_HEIGHT = 2160    # высота UHD 4K

INPUT_CSV = "qoe_ultimate.csv"
OUT_PNG = "qoe_average_ladder_lagrange_compromise.png"
OUT_STATS = "qoe_average_ladder_lagrange_compromise_stats.csv"
OUT_INTERP = "qoe_average_ladder_lagrange_compromise_interpolated_points.csv"
...
# 4096×2160 специально исключён: максимум экранной 4K-интерпретации — 3840×2160.
REFERENCE_LEVELS = [
...
pd.DataFrame(stats_rows).to_csv(OUT_STATS, index=False, encoding="utf-8-sig")
...
# Ручные поправки подписей, если при вставке в слайд где-то пересечётся текст.
...
print("\nКонтроль верхней границы нормировки:")
print(f"  max(raw_width) = {df['raw_width'].max():.1f} px")
print(f"  max(width_norm) = {df['width'].max():.1f} px")
if df["width"].max() > DISPLAY_MAX_WIDTH + 1e-9:
    raise ValueError(f"Ошибка: после нормировки найдено значение выше {DISPLAY_MAX_WIDTH}px")
else:
    print(f"  ✓ Все нормированные значения ≤ {DISPLAY_MAX_WIDTH}px")

print("\nДиагностика средних точек Main:")
for mos in MOS_SCORES:
    print(f"  MOS={mos}: Main ≈ {S['Main'][mos]['mean']:.1f} px")
...
# Визуальные сдвиги нужны только для разнесения кривых на рисунке.
# В расчёте полинома участвуют настоящие MOS-точки [5, 4, 3, 2].
OFFSETS = {
    "4K": -0.11,
    "1080": 0.00,
    "720": 0.11,
...
for mode in PLOT_MODES:
    ...
    label_name = (
        "Основная QoE-кривая (среднее тестовых режимов)"
        if is_main else MODE_LABELS[mode]
    )
    ...
    marker_face = color if is_main else "white"
    marker_edge = "white" if is_main else color
    marker_size = 10 if is_main else 8
    ...
    # ДИ показываем только для режимных кривых. Для Main не рисуем,
    # чтобы не создавать ложное впечатление отдельного измеренного ряда.
    if not is_main:
        ax.errorbar(
            xv_display,
            yv,
...
ax.set_title(
    "Формирование базовой QoE-лестницы: усреднение тестовых режимов\n",
    fontsize=15,
    fontweight="bold",
    color="#222222",
    pad=25,

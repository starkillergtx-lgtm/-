...
COLORS     = {'4K': '#1A6FBF', '1080': '#1E8A3A', '720': '#CC2020', 'Main': '#111111'}
LINE_STYLES = {'4K': '-', '1080': '--', '720': '-.', 'Main': '-'}
MOS_LABELS = {5: '5\n(отлично)', 4: '4\n(хорошо)', 3: '3\n(удовл.)', 2: '2\n(плохо)'}
...
def draw_itu(ax, y_bot, y_top):
    # ИЗМЕНЕНИЕ: Отодвинули текст правее, чтобы дать воздуху подписям точек
    x_pos = 1.28  
...
y_all = [S[fmt][m]['mean'] for fmt in PLOT_FORMATS for m in MOS_SCORES if not np.isnan(S[fmt][m]['mean'])]

Y_BOT = 0
Y_TOP = int(np.ceil(max(y_all) * 1.15 / 500.0) * 500)

# ИЗМЕНЕНИЕ: Расширили границу холста справа (было 1.45, стало 1.25)
X_R, X_L = 5.55, 1.25
...
OFFSETS = {'4K': -0.18, '1080': -0.06, '720': 0.06, 'Main': 0.18}
...
for fmt in PLOT_FORMATS:
    ...
    ax.plot(xd, cs(xd), color=color, ls=ls_style, lw=line_w, alpha=line_alpha, solid_capstyle='round', zorder=z_line, label=label_name)
    ...
    ax.plot(xv, yv, marker='o', color=color, markersize=m_size, lw=0,
            markerfacecolor=m_face, markeredgecolor=m_edge, markeredgewidth=2.2, zorder=z_mark)
    
    if not is_main:
        ax.errorbar(xv, yv, yerr=cv, fmt='none', ecolor=color, alpha=0.75,
    ...
    for xi, yi, mos in zip(xv, yv, MOS_SCORES):
        ...
        if (fmt, mos) in CUSTOM_LABEL_POSITIONS:
            ...
            if fmt == 'Main':      
                dx, dy = -12, 12; ha, va = 'right', 'bottom'
            elif fmt == '720':     
                dx, dy = -8, -14; ha, va = 'right', 'top'
            elif fmt == '1080':    
                dx, dy = 8, -14;   ha, va = 'left', 'top'
            elif fmt == '4K':      
                dx, dy = 12, 12;   ha, va = 'left', 'bottom'
        ...
        ax.annotate(f'{yi:.0f}', xy=(xi, yi), xytext=(dx, dy),
                    ...
                    fontsize=f_size, color=color, fontweight=font_w,
                    bbox=dict(boxstyle='square,pad=0.2', facecolor='white',
                              edgecolor=color, lw=1.2 if is_main else 0.8, alpha=0.95), zorder=11)
...
ax.set_title('Формирование базовой лестницы QoE: усреднение форматов экрана\n', 
             fontsize=15, fontweight='bold', color='#222222', pad=25)
...
# ИЗМЕНЕНИЕ: Перенесли легенду в левый нижний угол (в пустую зону)
leg = ax.legend(fontsize=10.5, loc='lower left', bbox_to_anchor=(0.02, 0.02),
                framealpha=0.95, edgecolor='#BBBBBB', borderpad=0.8, labelspacing=0.6, handlelength=1.5)
for line in leg.get_lines():
    line.set_linewidth(3.0) 

plt.tight_layout()
plt.savefig('qoe_average_ladder_final.png', dpi=200, bbox_inches='tight', facecolor='white', edgecolor='none')
plt.show()

print("\n✓ График формирования лестницы сохранён: qoe_average_ladder_final.png")
files.download('qoe_average_ladder_final.png')
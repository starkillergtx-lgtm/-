...
widths     = np.array([4000, 2087, 1046, 610, 516, 293])
heights    = np.array([2109, 1100, 551, 322, 272, 154])
labels     = [f"{w}×{h}" for w, h in zip(widths, heights)]

# Опорные экспериментальные точки, по которым строится полином 3-й степени.
# Субуровни 4.5 и 3.5 являются значениями этого полинома; уровень 2.5 исключён из итоговой лестницы.
...
def lagrange_eval(x_points, y_points, x_eval):
    ...
    - промежуточные субуровни MOS=4.5 и 3.5 рассчитаны по этому полиному; уровень MOS=2.5 исключён из итоговой лестницы.
    """
...
# Промежуточные субуровни MOS=4.5 и MOS=3.5 нанесены как значения этого полинома; уровень MOS=2.5 исключён из итоговой лестницы.
xd, yd = lagrange_curve(base_mos_points, base_widths, num=500)

# Строгая глубокая черная линия
MASTER_COLOR = '#1A1A1A'
ax.plot(xd, yd, color=MASTER_COLOR, lw=3.5, label='Финальная перцептивная QoE-кривая (полином Лагранжа)', zorder=4)
...
# Заголовки
ax.set_title('График 3: Итоговая перцептивная QoE-лестница (6 уровней)\n'
...
print("  Субуровни рассчитаны по полиному, PCHIP / сплайн не используется; MOS=2.5 исключён из итоговой лестницы")
...
w_excluded = lagrange_value_at(base_mos_points, base_widths, 2.5)
print(f"  MOS=2.5: ширина ≈ {w_excluded:.1f} px, округление: {int(round(w_excluded))} px - исключён из итоговой лестницы")

# ── 7. Вывод и сохранение ───────────────────────────────────────────────
plt.tight_layout()
plt.savefig('qoe_step4_final_6levels_lagrange_poly.png', dpi=180, bbox_inches='tight')
plt.show()

print("\n✓ График 3 успешно сгенерирован и сохранён как: qoe_step4_final_6levels_lagrange_poly.png")
files.download('qoe_step4_final_6levels_lagrange_poly.png')
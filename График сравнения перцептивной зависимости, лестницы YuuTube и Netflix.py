...
our_widths = np.array([4000, 2114, 1046, 708, 516, 293])
our_heights = np.array([2109, 1115, 551, 373, 272, 154])
...
our_full_names = [
    ...
    'MOS 4.5 - очень хорошо - 2114×1115',
    'MOS 4.0 - хорошо - 1046×551',
    'MOS 3.5 - приемлемо - 708×373',
    'MOS 3.0 - удовлетворительно - 516×272',
    'MOS 2.0 - плохо - 293×154'
]

sort_idx = np.argsort(mos_base)
pchip = PchipInterpolator(mos_base[sort_idx], width_base[sort_idx])

x_curve = np.linspace(2.0, 5.0, 900)
y_curve = pchip(x_curve)
...
def width_to_mos(width):
    ...
    return brentq(lambda x: float(pchip(x)) - width, 2.0, 5.0)

# ─────────────────────────────────────────────────────────────────────
# 2. Уровни YouTube и Netflix
# ─────────────────────────────────────────────────────────────────────
...
def make_ladder_df(ladder_name, levels):
    ...
    for item in levels:
        ...
        rows.append({
            ...
            'Ширина_px': w,
            'Высота_px': h,
            'MOS_по_зависимости': mos,
            'Комментарий': item['note']
        })

    df = pd.DataFrame(rows)
    df['ΔMOS_к_следующему'] = df['MOS_по_зависимости'].diff(-1).abs()
    return df
...
df_our = pd.DataFrame({
    ...
    'MOS_по_зависимости': our_mos,
    'Комментарий': ['уровень итоговой перцептивной лестницы'] * len(our_mos)
})
df_our['ΔMOS_к_следующему'] = df_our['MOS_по_зависимости'].diff(-1).abs()
...
df_all['MOS_по_зависимости'] = df_all['MOS_по_зависимости'].round(3)
df_all['ΔMOS_к_следующему'] = df_all['ΔMOS_к_следующему'].round(3)

df_all.to_csv('comparison_ladders_mos_full.csv', index=False, encoding='utf-8-sig')

print('\nСопоставление уровней по построенной зависимости разрешение(MOS):')
...
OUR_LABEL_POS = {
    5.0: (-8, -24, 'right', 'top'),
    4.5: (12, 12, 'left', 'bottom'),
    4.0: (-10, 12, 'right', 'bottom'),
    3.5: (10, 16, 'left', 'bottom'),
    3.0: (-12, 18, 'right', 'bottom'),
    2.0: (-16, 16, 'right', 'bottom')
}

YOUTUBE_LABEL_POS = {
    3840: (-10, -22, 'right', 'top'),
    2560: (-12, -18, 'right', 'top'),
    1920: (-12, -20, 'right', 'top'),
    1280: (-12, -24, 'right', 'top'),

    # Нижний диапазон: разводим по этажам
    854:  (-14, 18, 'right', 'bottom'),
    640:  (-14, -24, 'right', 'top'),
    426:  (-14, 20, 'right', 'bottom')
}

NETFLIX_LABEL_POS = {
    1920: (12, 12, 'left', 'bottom'),
    1280: (12, 14, 'left', 'bottom'),
    720:  (12, -20, 'left', 'top'),

    # Нижний диапазон: вправо и выше, кроме самого нижнего
    512:  (16, 24, 'left', 'bottom'),
    384:  (20, 12, 'left', 'bottom'),
    320:  (-18, -26, 'right', 'top')
}
...
# ─────────────────────────────────────────────────────────────────────
# 6. Справочные горизонтальные уровни разрешения
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# 7. Авторская зависимость и итоговые уровни
# ─────────────────────────────────────────────────────────────────────
...
for x, y in zip(our_x_plot, our_widths):
    ax.hlines(y, x, X_R, colors=COLORS['ours'], linestyles=':', lw=1.0, alpha=0.32, zorder=2)
    ax.vlines(x, Y_BOT, y, colors=COLORS['ours'], linestyles=':', lw=1.0, alpha=0.32, zorder=2)
...
# ─────────────────────────────────────────────────────────────────────
# 8. Функция отрисовки внешних лестниц
# ─────────────────────────────────────────────────────────────────────
...
def plot_external_ladder(levels, color, marker, line_style, offset, label_positions):
    ...
    for x, y in zip(plot_mos, widths):
        ...
        ax.hlines(y, x, X_R, colors=color, linestyles=':', lw=0.9, alpha=0.28, zorder=2)
        ax.vlines(x, Y_BOT, y, colors=color, linestyles=':', lw=0.9, alpha=0.28, zorder=2)
...
plt.figtext(
    0.43,
    0.925,
    'PCHIP-интерполяция по MOS-порогам  |  Серые зоны: уровни MOS  |  '
    'Горизонтальный сдвиг рядов носит только визуальный характер',
    ha='center',
    fontsize=10.5,
    color='#555555'
)
...
legend_handles = [
    ...
    Line2D([0], [0], color=COLORS['youtube'], lw=2.4, ls=LINE_STYLES['youtube'],
           marker='s', markersize=7, markerfacecolor='white',
           markeredgecolor=COLORS['youtube'], label='YouTube: стандартные уровни 16:9'),
    Line2D([0], [0], color=COLORS['netflix'], lw=2.4, ls=LINE_STYLES['netflix'],
           marker='D', markersize=7, markerfacecolor='white',
           markeredgecolor=COLORS['netflix'], label='Netflix: кандидатные уровни per-title'),
...
plt.savefig(
    'qoe_compare_youtube_netflix_balanced_lower_labels.png',
    dpi=200,
    bbox_inches='tight',
    facecolor='white',
    edgecolor='none'
...
print('\n✓ График сохранён: qoe_compare_youtube_netflix_balanced_lower_labels.png')
print('✓ Таблица сохранена: comparison_ladders_mos_full.csv')

files.download('qoe_compare_youtube_netflix_balanced_lower_labels.png')
files.download('comparison_ladders_mos_full.csv')
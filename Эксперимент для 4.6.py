...
#
# Ожидаемый вход:
#   C:\video_exp\originals\reference_1.mp4
#
# Основная папка результатов:
#   C:\video_exp\channel_ladder_test_4_6_full90
#
...
# Здесь уже лежат результаты полного 90-секундного прогона.
# Если channel_ladder_bitrates.csv есть, кодирование будет пропущено.
OUTPUT_DIR = Path(r"C:\video_exp\channel_ladder_test_4_6_full90")
...
PROPOSED_LADDER = [
    {"level_id": "P1", "mos": 5.0, "width": 4000, "height": 2109, "label": "4000x2109"},
    {"level_id": "P2", "mos": 4.5, "width": 2087, "height": 1100, "label": "2087x1100"},
    {"level_id": "P3", "mos": 4.0, "width": 1046, "height": 551,  "label": "1046x551"},
    {"level_id": "P4", "mos": 3.5, "width": 610,  "height": 322,  "label": "610x322"},
    {"level_id": "P5", "mos": 3.0, "width": 516,  "height": 272,  "label": "516x272"},
    {"level_id": "P6", "mos": 2.0, "width": 293,  "height": 154,  "label": "293x154"},
]
...
WIDTH_BASE = np.array([4000.0, 1046.0, 516.0, 293.0])
...
SCENARIOS = [
    {
        ...
        "description": "Высокий канал откалиброван по proposed P1 и YouTube Y1. Проверяется поведение лестниц в верхней области качества, включая около-4K уровни.",
...
def plot_scenario_channel_and_bitrate(timeline, scenario_id, main_figure=False):
    ...
    for df, label, color in [
        ...
        for row in switch_rows(df, "selected_level_id"):
            ...
            # Так плашки 720p/1280x720 и 1046x551 не налезают друг на друга.
            if (

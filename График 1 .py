...
if 'Статус' in df_all.columns:
    ...
    def tc_sec_f(tc):
        try:
            if str(tc)=='None': return np.nan
            p=str(tc).split(':'); return int(p[0])*60+int(p[1])
...
MOS_SCORES = [5, 4, 3, 2]
FORMATS    = ['4K', '1080', '720']
COLORS     = {'4K': '#1A6FBF', '1080': '#1E8A3A', '720': '#CC2020'}

# ТЕКСТУРНОЕ КОДИРОВАНИЕ ДЛЯ ЧБ-ПЕЧАТИ (Разная разметка линий)
LINE_STYLES = {'4K': '-', '1080': '--', '720': '-.'}
...
for fmt in FORMATS:
    ...
    for mos in MOS_SCORES:
        ...
        if len(group) == 0:
            ...
            # Средние значения для подписи точки: по всем очищенным наблюдениям
            mw = group['width'].mean()
            mh = group['height'].mean()
            n_obs = len(group)

            # Для ДИ: сначала среднее по каждому участнику
...
def draw_single(ax, fmt, show_ylabel=True):
    ...
    # Изменено: правая граница сдвинута на 1.15
    X_R, X_L = 5.55, 1.15
    ax.set_xlim(X_R, X_L)
    ...
    # ── Реальные наблюдения (по замечанию руководителя) ──
    mask_fmt = (
        df['Formatted_screen_size' if 'Formatted_screen_size' in df.columns else 'Формат_экрана']
        == fmt
    )
    ...
    # Проекции сделаны тоньше (lw=1.0) для ЧБ-аккуратности
    ...
    xd = np.linspace(2, 5, 500)
    ax.plot(xd, cs(xd), color=color, ls=ls_style, lw=2.8, solid_capstyle='round', alpha=0.95, zorder=4)

    # Везде круглые маркеры с мощным белым ореолом против слияния с усами
    ax.plot(xv, yv, marker='o', color=color, markersize=11, lw=0,
            markeredgecolor='white', markeredgewidth=2.2, zorder=5)
    
    ax.errorbar(xv, yv, yerr=cv, fmt='none', ecolor=color,
                elinewidth=2.5, capsize=8, capthick=2.5, zorder=6)

    for xi, yi, lbl in zip(xv, yv, labels):
        if np.isnan(yi): continue
        ax.annotate(lbl, xy=(xi, yi), xytext=(0, 12),
                    textcoords='offset points', ha='center', va='bottom',
                    fontsize=9, color=color, fontweight='bold',
                    bbox=dict(boxstyle='square,pad=0.25', facecolor='white',
                              edgecolor=color, lw=1.2, alpha=0.95), zorder=7)
...
# ── 8. Функция построения сводного графика (МАКС-НАГЛЯДНОСТЬ) ──
def draw_combined(ax):
    y_all = [S[fmt][m]['mean'] for fmt in FORMATS for m in MOS_SCORES if not np.isnan(S[fmt][m]['mean'])]
    ...
    for mos in MOS_SCORES:
        ax.axvspan(mos - LANE_HALF_WIDTH, mos + LANE_HALF_WIDTH, 
                   facecolor='#F0F0F0', edgecolor='#D5D5D5', lw=1.0, alpha=0.75, zorder=1)
        ax.axvline(mos, color='#999999', lw=1.2, ls=':', zorder=1)

    # МАКСИМАЛЬНОЕ РАЗДВИЖЕНИЕ КРИВЫХ К КРАЯМ ДОРОЖЕК (Сдвиг 0.11)
    OFFSETS = {'720': 0.11, '1080': 0.0, '4K': -0.11}
    ...
    # Отрисовка сплайнов с разной разметкой (ЧБ-Ready)
    for fmt in FORMATS:
        color = COLORS[fmt]; ls_style = LINE_STYLES[fmt]
        ...
        ax.plot(xd, cs(xd), color=color, ls=ls_style, lw=2.5, alpha=0.95, solid_capstyle='round', zorder=3)
        
        # Только круглые маркеры с белым прорезающим усы кантом
        ax.plot(xv, yv, marker='o', color=color, markersize=9, lw=0,
                markeredgecolor='white', markeredgewidth=2.0, zorder=5, label=fmt)
        
        ax.errorbar(xv, yv, yerr=cv, fmt='none', ecolor=color,
                    elinewidth=2.2, capsize=6, capthick=2.2, zorder=6)

        # Текстовые плашки разнесены с учетом расширенных сдвигов
        for xi, yi, mos in zip(xv, yv, MOS_SCORES):
            if np.isnan(yi): continue
            
            if fmt == '720':      
                dx, dy = -13, 14; ha, va = 'right', 'bottom'
            elif fmt == '1080':    
                dx, dy = 0, -18;   ha, va = 'center', 'top'
            else:                 
                dx, dy = 13, 14;   ha, va = 'left', 'bottom'
                
            ax.annotate(f'{yi:.0f}', xy=(xi, yi), xytext=(dx, dy),
                        textcoords='offset points', ha=ha, va=va,
                        fontsize=8, color=color, fontweight='bold',
                        bbox=dict(boxstyle='square,pad=0.15', facecolor='white',
                                  edgecolor=color, lw=0.8, alpha=0.95), zorder=7)
    ...
    # В легенду автоматически добавятся линии с разным стилем разметки
    ax.legend(title='Формат экрана', fontsize=10, title_fontsize=10, loc='upper right', framealpha=0.95, edgecolor='#BBBBBB')

# ── 9. Компоновка панели 2×2 ──────────────────────────────────
...
fig.suptitle(
    ...
    fontsize=12, y=1.01, color='#222222')
...
plt.savefig('qoe_ultimate_bw_perfect.png', dpi=180, bbox_inches='tight', facecolor='white', edgecolor='none')
plt.show()

print("\n✓ Абсолютный мастер-шаблон сохранён: qoe_ultimate_bw_perfect.png")
files.download('qoe_ultimate_bw_perfect.png')